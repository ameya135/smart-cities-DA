...
class RNN:
    ...
    def prediction_interval(self, X_train, y_train, x0, path=rf'{os.getcwd()}'):
        ...
        for b in range(n_boots):
            ...
            model.fit(X_train[train_idx], y_train[train_idx], epochs=10, verbose=0, validation_data=(X_train[val_idx], y_train[val_idx]),
...
class CVTuner(kt.engine.tuner.Tuner):
    ...
    def save_model(self, trial_id, model, step=0):
        """Save a model based on the trial_id and step.
        
        Args:
            trial_id: The identifier of the trial for which the model is saved.
...
class VanillaRNN(RNN):
    ...
    def fit(self, X, y, epochs, max_trials, units=[10, 100], act=['tanh', 'relu'], layers=[1, 2], lr=[0.1, 0.01, 0.001]):
        ...
        tuner = CVTuner(hypermodel=RNN_HyperModel(mtype='SimpleRNN', input_shape=(X.shape[1], X.shape[2]), units=[10,100],
                            ...
                            oracle=kt.oracles.BayesianOptimizationOracle(objective='val_loss', max_trials=max_trials),
                            directory=os.getcwd(),
